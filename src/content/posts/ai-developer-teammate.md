...
이 원칙엔 대가가 있다 — 독립 검증을 의무화하면 작업마다 확인 명령(검색·빌드·테스트)을 돌리는 시간이 얹힌다. 그래도 '고쳤다는 착각'을 그대로 배포하는 것보다는 싸다고 봤다.
...
- **명세를 짓지 않는다** — 무엇을·왜는 기획(사람 또는 기획자 팀원) 몫이다. 개발자는 모호한 명세를 채워 넣지 않고 되묻는다.
