...
**하네스 자체의 신뢰성.** 하네스라는 말은 원래 테스트 하네스, 즉 테스트를 걸어 돌리는 틀에서 온 단어다. 그리고 초록불이 「통과」인지 「아무것도 안 본 것」인지 모른다는 테스트 하네스의 오래된 함정은 에이전트 하네스에도 그대로 있다. [수동 26건이 1건이 되자, 초록을 의심하기 시작했다](/posts/kiosk-screen-automation-renewal/)에서 겪은 그 문제다. 검증 에이전트를 붙이는 것과, 그 검증이 실제로 빨강을 낼 수 있는지 확인하는 것은 다른 일이고, 뒤쪽이 더 어렵다.
...
| 하네스 글들의 개념 | 우리가 이미 갖고 있던 것 |
| --- | --- |
| 생성과 평가의 분리 | 반증 서브에이전트, 역할별 세션 분리 |
| 관측·텔레메트리 | 세션 간 대화 다이제스트 |
| 하네스 자체의 신뢰성 | 「초록을 의심하다」 |
| sandbox·승인 경계 | 역할 경계, 배포 승인 게이트 |
