...
> 페르소나 시뮬레이션도 원리는 같다. 지식 소스를 명시적으로 제한해서(화면 정본·환경 사전만) "설계자가 아는 걸 모르는 사람"을 억지로 만든다. 격리가 핵심이다.
...
실제로 이 구조를 처음 돌린 첫 패스에서만 25건을 잡았다(신규 커버리지 4건, "실측으로 뭉갠" 게 실은 티켓에 확정돼 있던 스펙 3건, 도구 실수 1건 포함). 네 라운드까지 돌려 "수행 불가 33건이 0"이 된 전체 서사와 교차 세션 증거는 [본편](/posts/ai-confirmation-bias-subagents/)에 있다.
