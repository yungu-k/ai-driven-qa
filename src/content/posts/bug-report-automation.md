...
QA를 하다 보면 결함 자체를 찾는 시간보다 **결함을 리포트로 옮기는 시간**이 더 든다. Fail 하나가 나올 때마다 Jira를 열고, Summary를 규칙에 맞게 짓고, 환경·사전조건·재현 절차·기대 결과·실제 결과·관련 TC를 손으로 채운다. 신규 프로젝트 테스트 초반엔 버그가 하루에 수십 건씩 쏟아지는데, 한 건에 평균 5분이라 쳐도 수십 건이면 반나절이 증발한다.
