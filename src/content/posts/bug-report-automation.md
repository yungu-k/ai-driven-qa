...
QA를 하다 보면 결함 자체를 찾는 시간보다 결함을 리포트로 옮기는 시간이 더 든다. Fail 하나가 나올 때마다 Jira를 열고, Summary를 규칙에 맞게 짓고, 환경·사전조건·재현 절차·기대 결과·실제 결과·관련 TC를 손으로 채운다. 신규 프로젝트 테스트 초반엔 버그가 하루에 수십 건씩 쏟아지는데, 한 건에 평균 5분, 수십 건이면 반나절이 증발한다.
...
- 더 큰 이득은 시간이 아니라 일관성이다. 몇 건을 만들든 Summary 형식·Description 6섹션·라벨·버전이 동일하게 나온다. 검색·필터·집계가 처음부터 맞는다.
...
아직 사람 손이 남는 지점은 '실제 결과' 확인이다 — 다음은 실패 화면 스크린샷 첨부까지 자동으로 당길 수 있는지가 숙제다.
