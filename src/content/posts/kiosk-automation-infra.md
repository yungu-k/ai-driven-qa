---
author: 공윤구
pubDatetime: 2026-06-04T05:00:00.000Z
title: 테스트 1,160건으로 무인 키오스크를 지켰다
slug: kiosk-automation-infra
featured: false
draft: false
tags:
  - test-automation
description: 무인 키오스크 앱을 Web UI와 API 두 트랙으로 자동 검증하는 인프라. 소스 클론부터 티켓 등록까지 한 번의 명령으로.
---

> 키오스크 앱(현금↔칩 교환 무인기)을 Web UI와 API 두 트랙으로 자동 검증하는 인프라. 소스를 받아 테스트를 생성·실행하고, 실패를 리포트·Jira 티켓으로 연결하는 과정을 한 번의 명령으로 반복한다.

## 음수 금액이 200으로 통과하고 있었다

무인 키오스크는 현금과 칩을 사람 없이 주고받는다. 그 말은, 잘못된 응답 하나가 곧 현금 사고라는 뜻이다. 실제로 어떤 API는 **음수 금액을 넣어도 `200 OK`로 통과**시키고 있었다 — 그리고 아무도 그걸 매 릴리스마다 손으로 확인하지 못했다. 화면 32개 × 검증 수백 건을 사람이 매번 돌리는 건 불가능하니까.

그래서 두 트랙(Web UI + API) 자동 검증 인프라를 세웠다. 지금 규모는 이렇다:

<figure class="not-prose my-6 rounded-xl border border-border bg-muted/40 p-4" role="img" aria-label="산출물 규모 통계 카드. API 자동화는 1,160 테스트로 1,133 PASS와 27 manual-required, fail 0. Web UI 자동화는 2기종 32화면 128 spec. 자동 검출하고 수정한 production 부정합은 5종.">
  <div class="flex flex-wrap gap-3 text-xs">
    <div class="flex-1 rounded-lg border border-border bg-background/60 p-3"><p class="text-lg font-bold text-accent">1,160</p><p class="mt-1 text-muted-foreground">API 테스트 (1,133 PASS + 27 manual · fail 0)</p></div>
    <div class="flex-1 rounded-lg border border-border bg-background/60 p-3"><p class="text-lg font-bold text-accent">128</p><p class="mt-1 text-muted-foreground">Web UI spec (2기종 × 32화면 × 4종 검증)</p></div>
    <div class="flex-1 rounded-lg border border-border bg-background/60 p-3"><p class="text-lg font-bold text-accent">5</p><p class="mt-1 text-muted-foreground">자동 검출·수정한 production 부정합</p></div>
  </div>
</figure>

## 무엇을 만들었나

- **Web UI 트랙 (Playwright):** 화면 진입·사용자 흐름·다국어·시각 회귀 검증. 키오스크 2기종 약 32개 화면. 이후 리뉴얼로 사람 몫이던 26건 중 25건까지 흡수했고 거래 흐름 검사가 더해져 검사 구성 자체가 달라졌다 — 그 과정과 현재 수치는 [화면 자동화 리뉴얼기](/posts/kiosk-screen-automation-renewal)에 따로 담았다. (아래 카드의 128은 이 글 시점의 설계 계산치다.)
- **API 트랙 (vitest + supertest):** 디바이스 서비스 모듈 18종 + 백엔드 API의 응답 계약(contract)·에러 코드 정합 검증.
- **Orchestrator (Node + TypeScript):** 소스 clone → 테스트 생성 → 원격 실행 → 리포트 → 티켓 등록 5단계 파이프라인.

| 트랙 | 검증 목적 | 도구 |
| --- | --- | --- |
| Web UI | 화면 노출·사용자 흐름·다국어·시각 회귀 | Playwright |
| API | 모듈별 응답 계약·에러 코드 정합 | vitest + supertest + zod |

<figure class="not-prose my-6 rounded-xl border border-border bg-muted/40 p-4" role="img" aria-label="Orchestrator 5단계 파이프라인 도식. 소스 clone, 테스트 생성, 실행, 리포트, 티켓 등록이 순서대로 이어진다. 테스트 생성과 실행 단계는 Web UI 트랙(Playwright)과 API 트랙(vitest+zod) 두 갈래로 나뉘어 병렬로 진행된 뒤, 리포트 단계에서 다시 합쳐진다. 전 과정이 한 번의 명령으로 반복된다.">
  <figcaption class="text-xs font-semibold text-muted-foreground">Orchestrator — 한 번의 명령으로 도는 5단계, 가운데서 두 트랙으로 갈라진다</figcaption>
  <div class="mt-4 flex flex-wrap items-center gap-x-2 gap-y-2 text-xs">
    <span class="rounded-lg border border-border bg-background/60 px-3 py-2 text-muted-foreground">① 소스 clone</span>
    <span aria-hidden="true" class="text-muted-foreground">→</span>
    <div class="rounded-lg border border-accent/60 bg-accent/5 p-2">
      <p class="mb-1.5 text-center text-[10px] text-muted-foreground">② 생성 → ③ 실행 <span class="text-accent">(두 트랙 병렬)</span></p>
      <div class="flex flex-col gap-1.5">
        <span class="rounded-md border border-border bg-background/60 px-2 py-1 text-foreground">Web UI · Playwright <span class="text-muted-foreground">— 32화면</span></span>
        <span class="rounded-md border border-border bg-background/60 px-2 py-1 text-foreground">API · vitest+zod <span class="text-muted-foreground">— 18모듈</span></span>
      </div>
    </div>
    <span aria-hidden="true" class="text-muted-foreground">→</span>
    <span class="rounded-lg border border-border bg-background/60 px-3 py-2 text-muted-foreground">④ 리포트</span>
    <span aria-hidden="true" class="text-muted-foreground">→</span>
    <span class="rounded-lg border border-border bg-background/60 px-3 py-2 text-muted-foreground">⑤ 티켓 등록</span>
  </div>
</figure>

## 코드로 보는 두 트랙

**Web UI 트랙** — 화면마다 "UI 요소가 스펙대로 노출되는가"를 명시적 assertion으로 검증한다. 공통 helper로 다국어·접근성·시각 회귀를 표준화했다.

```ts
// screens/cashinput.spec.ts — 화면 spec 패턴 (발췌)
test("현금 투입 화면 — UI 요소 노출", async ({ page }) => {
  await gotoScreen(page, "/cashinput");

  // form 계약: action + hidden inputs
  const form = page.locator("form#cashForm");
  await expect(form).toHaveAttribute("action", "/cashcheck");

  // 핵심 요소 — 스펙 문서의 element 인벤토리 기반
  await expect(page.locator("#totalAmount")).toBeVisible();
  await expect(page.locator("img.machine-guide")).toHaveAttribute(
    "src", /cash_guide/
  );

  // 다국어 토글 — 언어 전환 후 문구키 검증
  await switchLanguage(page, "en");
  await expect(page.locator("h1")).toHaveText(/Insert Cash/);
});
```

**API 트랙** — 응답을 zod 스키마로 계약 검증한다. "필드가 있다"가 아니라 "타입·값 범위까지 스펙과 일치한다"를 본다.

```ts
// api-runtime/chip-dispenser.spec.ts — 계약 검증 패턴 (발췌)
const DispenseResponse = z.object({
  result: z.number(),          // 문자열 "0" 이 오면 여기서 잡힌다
  errorCode: z.string().regex(/^\d{6}$/),
  data: z.object({
    dispensedCount: z.number().int().nonnegative(),
  }),
});

it("칩 지급 — 정상 응답 계약", async () => {
  const res = await request(BASE).post("/api/chip/dispense")
    .send({ amount: 100 });

  expect(res.status).toBe(200);
  expect(res.headers["content-type"]).toMatch(/json/); // text/plain 회귀 방지
  DispenseResponse.parse(res.body); // 계약 위반 시 즉시 실패
});

it("칩 지급 — 음수 금액은 400", async () => {
  const res = await request(BASE).post("/api/chip/dispense")
    .send({ amount: -1 });
  expect(res.status).toBe(400); // 과거엔 200으로 통과했던 그 버그
});
```

실제로 위 패턴이 잡아낸 게 아래 결함들이다 — "숫자여야 할 result가 문자열", "음수가 200으로 통과".

## 자동화가 실제로 잡아낸 결함

- **응답 타입 불일치:** 일부 endpoint가 숫자여야 할 상태값을 문자열로 반환 → 프런트 비교 로직 우회 가능. 전수 number로 통일.
- **입력 검증 부재:** 음수·누락·null 파라미터가 모두 200으로 통과 → 검증 도입으로 400 정상화.
- **Content-Type 누락:** 한 endpoint만 text/plain 반환 → 거래 흐름 파싱 실패 원인. 수정 확인.
- **접근성 위반:** 버튼 라벨·이미지 대체텍스트 누락 등 자동 검출.

## 겪은 문제와 해결

### 1. Mock과 실제 응답의 부정합

디바이스 없는 환경에서 Mock 응답으로 테스트하다 보니 Mock과 실제 하드웨어 응답이 미묘하게 달랐다.

- **해결:** 에러 코드 카탈로그를 단일 출처로 정리하고, Mock에 시나리오 트리거를 심어 정상/에러 경로를 모두 재현. 발견한 부정합은 검출에 그치지 않고 코드 수정까지 연결.
- **그 뒤(2026-09):** Mock 체계 자체를 실물 응답을 기록해 재생하는 record & replay 방식으로 다시 지었다. 손으로 만든 응답 0건. 같은 스위트를 실물과 mock 양쪽에 돌려 결과가 같은지 확인하는 것까지가 원칙이 됐다 — 부정합을 겪어 본 결론이다.

### 2. 상시 빨간 fail → green baseline

초기엔 fail이 20건 넘게 쌓였는데, 점검 결과 단 하나도 실제 운영 결함이 아니었다(Mock 한계·타이밍). 상시 빨간 fail은 회귀 신호를 가리는 noise다.

- **해결:** 환경 한계 케이스를 manual-required로 분리하고, 안정적으로 통과하는 spec만 green baseline으로 묶어 회귀 게이트 신뢰도 확보.

| | 초기 | 정리 후 |
| --- | --- | --- |
| 상시 fail | 20건+ (전부 noise) | 0 |
| 실운영 결함 | 0 (fail 중) | — |
| manual-required | 미분리 | 27건 별도 격리 |

### 3. 환경 종속성 → Local-first 전환

실 키오스크에 의존하면 장비 점유·전원 상태에 자동화가 묶였다.

- **해결:** 앱을 로컬에서 직접 기동하도록 전환. 실 장비는 최종 sanity check에만 사용.

### 4. 비기술 사용자 접근성

CLI는 운영·비개발 인원이 쓰기 어려웠다.

- **해결:** 브라우저에서 모듈 그룹을 골라 실행하고 실시간 로그·결과를 보는 [Runner Web UI](/posts/api-runner-web-ui)를 추가.

## 핵심 교훈

> 자동화의 가치는 '통과'가 아니라 '신뢰할 수 있는 실패'에 있다. 항상 빨간 화면은 회귀 신호를 죽인다.

- 우회(Mock 변조)는 Web UI 검증엔 허용하되, API 정합 검증은 진짜 응답으로. 트랙별 책임을 분리해야 결과를 믿을 수 있다.
- 에러 코드·스펙은 단일 출처 문서로. 코드가 문서를 따라가야 지식이 코드에 갇히지 않는다.
- 부정합은 검출에서 멈추지 말고 수정까지. 이번 인프라에서는 검출한 부정합 5종이 전부 실제 코드 수정으로 이어졌다.

## 이 자동화가 보장하지 않는 것

- **27건은 자동화 밖이다** — Mock·환경 한계로 수동 검증(manual-required)에 남긴 케이스. 자동화가 "전부"를 검증한다고 말하지 않기 위해 분리해 둔 영역이다.
- **실 장비의 물리 동작은 대체하지 못한다** — 로컬 기동 기반이라, 실제 하드웨어 상호작용은 여전히 최종 sanity check에서 사람이 확인한다.
- **스펙 문서 자체가 틀리면 그 오류는 잡지 못한다** — 계약 검증이 보는 것은 "스펙과 일치하는가"까지다. 스펙이 곧 정답지라, 정답지의 오류는 이 자동화의 범위 밖이다.

이 인프라를 명령어가 아니라 버튼으로 돌리게 만든 다음 단계는 [API 실행 Web UI](/posts/api-runner-web-ui)에 담았다 — 회귀를 비기술 구성원도 누를 수 있게 한 이야기다. 화면 트랙이 그 뒤 어디까지 넓어졌고, 커진 초록을 어떻게 의심하게 됐는지는 [화면 자동화 리뉴얼기](/posts/kiosk-screen-automation-renewal)로 이어진다.

그리고 2026-09, 이 두 트랙 위에 새 층이 얹혔다 — 실물 응답을 기록·재생하는 공용 mock 서버, 24시간 주기의 상시 계약 검증, 젠킨스에서 새 빌드가 감지되면 diff 영향 분석→실행→시트 기입→실패 리포트→Jira 초안까지 사람 손 없이 도는 자동 파이프라인. 트랙 구조는 그대로 두고 실행 주체가 사람에서 파이프라인으로 넓어진 것이라, 이 글에 적은 설계는 지금도 그 밑에서 그대로 돈다. 이 글의 수치들은 구축 시점 실측이다.
