...
React 18 + Vite + TypeScript + Tailwind, 백엔드는 Supabase(Postgres · Auth · Storage · Edge Functions), 배포는 Vercel. 런타임 의존성 4개가 전부다. 기각한 것들이 스택을 설명한다.
...
현장 사진 버킷이 public 이었다. URL만 알면 누구나 열렸다. private 으로 바꾸고 만료형 signed URL로 전환한 뒤 구 공개 URL이 실제로 차단되는지 네거티브로 확인했다.
