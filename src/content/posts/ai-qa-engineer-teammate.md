...
AI 기획자를 먼저 만들고 나니 대칭되는 QA 팀원이 필요해졌다. 그런데 QA는 '반복'만 떼어 위임하기가 애매하다 — 무엇이 깨질지 짚는 건 판단이고, 판단까지 AI에 넘기면 '통과했다'는 착각을 그대로 배포하게 된다. 그래서 대체가 아니라 보강으로 선을 그었다. 이 글은 그 경계를 4층 구조에 어떻게 박았는지, 산출물 언어를 왜 독자별로 갈랐는지를 다룬다. QA 팀원은 예외·엣지·경계·상태전이를 먼저 짚어주고 테스트를 초안하는 조수다.

## 구성

기획자와 대칭되는 4층 구조로 짰다. 불변(헌법)과 가변(상태)을 분리하는 게 핵심 — 원칙은 고정하고, 프로젝트별 맥락만 갈아끼운다.

| 층 | 성격 | 담는 것 |
| --- | --- | --- |
| 페르소나 | 정체성 | "무엇이 깨질까"를 먼저 짚는 QA 조수 |
| 헌법 | 불변 | 1년치 실수 교정을 이식한 불변 규칙 |
| 방법론 | 준불변 | 커버리지 조언·유닛 테스트·diff 영향도 3대 역할의 절차 |
| 상태 | 가변 | 지금 프로젝트·대상 스펙·진행 맥락 |

**3대 역할:**

1. **테스트 커버리지 조언** — 예외·경계 먼저 제안, 정상 경로는 짧게
2. **유닛 테스트 작성** — 일부러 깨뜨려 red(테스트 실패 상태) 확인하는 역검증 필수
3. **diff 기반 영향도 테스트**

**핵심 설계 원리 = 산출물마다 언어가 갈린다.** 테스터용(TC·조언)은 코드 용어를 금지하고 메뉴·화면·조작·결과로 풀어쓴다. CI용(유닛테스트·diff노트)은 코드 용어를 허용한다. 기획자는 항상 쉬운 말이지만, QA는 독자를 따라 언어를 분기한다.
...
대가는 있다. 지식베이스를 하나로 묶으면 그 한 곳이 흔들릴 때 팀원도 같이 흔들린다 — QA 허브 규격을 고치면 QA 팀원의 판단 기준이 즉시 따라 바뀐다. 그래도 두 벌을 각자 관리하다 어긋나는 것보다는, 단일 출처의 이 위험을 택했다.
...
- **유닛 테스트는 역검증 필수** — 일부러 코드를 깨뜨려 red를 확인해야 '항상 통과하는 가짜 테스트'를 걸러낸다. 이건 [AI가 자기 산출물을 무조건 통과시키는 확증편향](/posts/ai-confirmation-bias-subagents/)을 코드 레벨에서 막는 장치다 — green만 보고 "됐다"는 AI를 믿지 않고, red가 나오는지를 강제로 확인시킨다.

## 이 팀원이 대신 못 하는 것

- **실장비 실행은 사람 몫** — QA 팀원은 예외·경계를 짚고 테스트를 초안하지만, 실제 키오스크에서 돌려 물리 동작을 확인하는 건 여전히 사람이 한다.
- **스펙의 옳고 그름은 판단하지 않는다** — "스펙대로 검증"까지가 경계다. 스펙 자체가 틀렸는지는 사람이 결정한다.
- **도메인 암묵지에 의존한다** — 헌법에 이식한 건 지난 1년의 실수 교정이다. 문서에 없는 새 맥락은 사람이 먼저 채워줘야 팀원이 참조한다.
