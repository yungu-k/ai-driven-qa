...
새 화면 시안이 나오면 QA는 그걸 테스트 케이스로 번역한다. 화면 하나에 약 10행(체감치) — 정상 경로, 예외, 경계, 상태 전이를 뽑아 쓰고, 그 위에 Priority·드롭다운·조건부 서식·테두리·틀 고정까지 손으로 입힌다. TC 내용을 짜는 것보다 **시트를 사람이 볼 수 있는 문서로 만드는 서식 노동**이 시간을 더 먹는 날도 많았다. 이 글은 그 반복을 규격 문서 하나로 걷어낸 과정을 다룬다 — 무엇을 자동화했고, 어디까지는 사람 몫으로 남겼는지까지.
...
**1. 서식이 매번 빠지거나 덮어써졌다.** 데이터만 넣고 서식을 빠뜨리거나, 적용 순서가 틀려 뒤 단계가 테두리를 덮어썼다. → **9단계 체크리스트**로 순서를 고정. 특히 Border는 마지막 단계로 못박아 해결.
