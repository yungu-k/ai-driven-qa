...
tags:
  ...
  - report
  - jira
description: QA 리포팅을 데이터 수집→집계→시트→PDF→메일 파이프라인으로 규격화하고, 사람은 판정(Verdict)과 승인만 하게 만든 이야기. 규격이 곧 AI의 작업 지시서다.
---
...
예전엔 QA가 끝나면 결과를 **메일 본문에 서술**해서 보냈다. "TC 몇 건 중 몇 건 Pass, 결함은 몇 건…"을 매번 손으로 풀어 썼다. 이력이 안 남는 건 아니다 — 메일함엔 남는다. 진짜 함정은 다른 데 있었다. **한 통 쓰는 데 시간이 오래 걸리고**, 표준 보관처 없이 메일함 여기저기 흩어져 검색·비교가 어렵고, 릴리스마다 형태가 제각각이었다.
