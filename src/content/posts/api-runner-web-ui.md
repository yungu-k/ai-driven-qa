...
비기술 사용자용 도구라면 exe 배포도 후보였다. 하지만 데스크톱 프레임워크(Electron류)는 번들이 크고 업데이트 배포가 번거롭다. 셸 GUI는 유지보수가 어렵다. 결론은 기존 자동화와 같은 언어·같은 런타임 위의 **경량 웹 서버 + 순수 HTML/JS**. 프론트 프레임워크도 안 썼다 — 화면 세 장에 빌드 체인은 과하다.
...
2. **서버 + 테스트 러너 spawn** — 웹에서 고른 모듈만 기존 테스트 러너를 자식 프로세스(child process)로 띄운다. 러너를 새로 짜지 않고 spawn으로 재사용한 건, CLI와 웹이 **같은 실행 경로·같은 리포트 저장소**를 공유해야 결과를 믿을 수 있어서다. 대신 자식 프로세스 stdout이 버퍼링돼 로그가 뭉텅이로 늦게 나오는 문제가 있어, 버퍼를 즉시 흘려보내는 설정을 따로 잡았다
3. **실시간 스트림(SSE)** — 로그를 polling으로 긁을 수도 있었지만 SSE(Server-Sent Events)를 골랐다. 회귀는 수 분씩 걸리는데 polling은 간격만큼 로그가 밀리고 서버에 빈 요청을 계속 던진다. SSE는 서버가 로그가 생기는 대로 밀어주니 '돌아가고 있나?' 불안이 사라진다
...
이 글의 러너는 첫 버전 기록이다 — 그 뒤 두 번 자랐다. 먼저 Web UI(화면) 트랙까지 품는 통합 UI가 됐고(「지금 검증하는 대상」 표시 같은 새 장치는 [화면 자동화 리뉴얼기](/posts/kiosk-screen-automation-renewal)의 사건 3에), 2026-09에는 사람이 누르는 실행 위에 자동 실행이 얹혔다. 젠킨스에서 새 빌드가 감지되면 검증·시트 기입·실패 리포트까지 파이프라인이 스스로 돌고, 첫 화면도 실행 메뉴가 아니라 두 트랙의 최신 결과를 한눈에 보는 「현황」 페이지가 됐다. 「버튼을 누를 수 있는 사람」에서 「버튼을 안 눌러도 되는 팀」으로 — 다만 자동 실행은 사람 실행에 양보하고, Jira 결함 등록은 여전히 사람 승인 뒤에만 나간다.
