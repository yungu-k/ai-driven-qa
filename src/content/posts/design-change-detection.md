---
author: 공윤구
pubDatetime: 2026-04-10T09:19:00.000Z
title: 디자인 변경 감지 + TC 커버리지 갭 분석
slug: design-change-detection
featured: false
draft: false
tags:
  - qa-automation
  - figma
  - coverage
description: 디자인 파일의 노드 트리 스냅샷 diff로 변경 화면을 자동 식별하고 TC 커버리지 갭을 리포트하는 시스템.
---

> 디자인 변경을 자동 감지하고, TC 시트와 비교하여 커버리지 갭을 리포트하는 워크플로우.

## 무엇을 만들었나

디자인 도구의 REST API를 활용한 스냅샷 기반 변경 감지 시스템. 페이지별 노드 트리를 JSON으로 저장하고, 이전 스냅샷과 비교하여 추가/삭제/변경된 화면을 자동 식별한다.

- 스냅샷 저장/로드/비교 유틸리티 (약 230줄)
- 변경 감지 + TC 커버리지 갭 분석 슬래시 커맨드
- 대상 페이지는 프로젝트별로 설정 가능

## 어떤 문제를 겪었나

### 1. 비-UI 노드의 오탐지

디자인 페이지에는 comment, MEMO, sticker 등 실제 UI가 아닌 디자이너의 메모가 프레임으로 존재한다. 이것들이 변경 감지에 잡히면 불필요한 리포팅이 발생했다.

- **해결:** 스킵 이름 + 스킵 접두사 목록으로 필터링. 프로젝트별로 확장 가능한 구조.

### 2. 되돌리기(Ctrl+Z)의 오탐지 가능성

디자이너가 수정 후 되돌리면 수정 시각은 변경되지만 실제 노드 트리는 동일하다.

- **해결:** 시각 비교가 아닌 노드 트리 diff 방식 채택. 실질적 변경이 없으면 '변경 없음'.

### 3. 중첩 구조 내부 프레임 누락

페이지 > 섹션 > 프레임 계층인 경우, 단순 순회로는 섹션 내부의 프레임을 놓쳤다. 재귀적 추출로 해결.

### 4. 스케줄링 vs 수동 실행

정기 스케줄링을 고려했으나 디자인 업데이트 빈도가 불규칙해 불필요한 실행이 많았다. 수동 커맨드로 결정.

## 핵심 교훈

> 변경 감지는 'timestamp 비교'보다 '실질적 diff'가 정확하다. 노드 트리 diff 방식이 오탐을 걸러준다.

- 비-UI 노드 필터 목록은 프로젝트별로 다르다. 쉽게 확장 가능한 구조가 중요하다.
- 자동화의 실행 주기도 설계의 일부. 무조건 스케줄링이 아니라 사용 패턴에 맞는 트리거를 선택해야 한다.
