...
[앞 글](/posts/release-train-scrum/)에서 열차 체계를 설계했다. 그 설계를 Jira에 옮기는 순간 첫 오해가 드러났다. 버전 항목이 80개 넘게 난립해 있었고, 릴리스 대상 티켓 53건은 어느 보드에도 제대로 모여 있지 않았다. 전환 전에는 "릴리스 묶음"과 "QA"를 각각 에픽으로 만들어 관리하고 있었다.
...
- **보드에 무엇을 올릴지는 라벨로 지정**했다. 에픽 기준은 에픽이 계속 늘어 필터가 쉽게 깨지고, 다른 필드는 이미 오염됐거나 요금제 문제로 탈락했다. 라벨의 약점도 그대로 적어둔다 — **라벨이 빠지면 티켓이 보드에서 실종된다.** 그래서 자동 부여 규칙과 '누락 색출 필터'를 같이 만들었다.
...
1. **에픽 하나가 한 번의 반복이다** → 아니다 (에픽:스프린트는 N:M — 위 "에픽은 스프린트를 담는 그릇이 아니다" 참조).
2. **고객 배포용 버전을 따로 만들어야 한다** → 아니다 (누적이라 최신 검증본 하나가 이전을 다 포함한다 — 자세한 이유는 [앞 글의 '반직관 지점'](/posts/release-train-scrum/)에).
...
5. **보드 컬럼만 나누면 된다** → 아니다. 컬럼은 상태를 비추는 거울일 뿐이다. 거울을 늘린다고 상태가 생기지 않는다 — 컬럼 두 개를 원하면 상태 두 개가 필요하다.
6. **일단 다 정리하고 시작하자** → 아니다. 한 번에 뒤엎지 않는다. 진행 중인 건은 기존 방식대로 마무리하고, 다음 주기부터 새 방식을 적용했다.

정리하다 드러난 것들도 있다. 버전 항목이 80개 넘게 난립해 있었고(모듈 버전이 릴리스 버전 자리에 들어와 있었다) — 릴리스 버전을 고르는 드롭다운 하나에 몇 년치 이력이 뒤섞여 있던 셈이다. 종결 사유엔 같은 이름이 중복으로 존재했고, 한 보드에 서로 다른 제품 라인이 섞여 있었다. 프로세스를 세우는 일의 절반은 **쌓인 것을 걷어내는 일**이었다.

## 남은 것

보드도 규칙도 세웠지만, 라벨 하나가 빠지면 티켓이 보드에서 사라지는 구조다. 정착은 이 규칙들이 손에 붙어 반사신경이 될 때까지 반복이 만든다 — '누락 색출 필터'를 매번 안 돌려도 되는 날이 오면 그때 끝난 것이다. 지금은 그 반복의 초입이다.

> 도구는 규칙을 담을 뿐, 규칙을 지키게 하진 않는다. 그건 반복이 한다.
