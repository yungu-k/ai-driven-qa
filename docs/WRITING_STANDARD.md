...
## 4-2. 쉬운 글 원칙 (R) — 2026-09-09 사용자 지시로 신설

> 사용자 원문: 「가볍고 읽기 쉽고, 이해가 잘되도록, 마치 중고등학생을 설명하는 듯한 블로그.」
> 이 원칙의 관리는 TW 의 역할 그 자체다(사용자 명시). AI 티 감사(§4-1)를 통과해도
> 이 절에 걸리면 다시 쓴다 — **압축은 티 제거의 목적이 아니다.**

- **R1 독자 기준**: 중·고등학생에게 설명하듯. 소리 내 읽어 한 번에 뜻이 안 들어오면 다시 쓴다.
- **R2 조어 금지**: 「팀원 축」 「위치 항상성」처럼 눌러 담은 명사구를 만들지 않는다.
  두 문장으로 풀면 조어가 필요 없다.
- **R3 명제문 뒤엔 구체문**: 「비어 있는 건 축이지 은유가 아니다」류의 추상 문장은
  단독으로 두지 않는다. 반드시 앞이나 뒤에 실물 예가 있어야 하고, 명제문을 지워도
  뜻이 통하면 명제문 쪽을 지운다.
- **R4 한 문장 한 생각**: 괄호 보충이 한 줄을 넘으면 문장을 쪼갠다.
- **R5 드리프트 경계**: 팀 내부 문체(압축 명제·내부 조어)는 세션 간 메시지용이다.
  블로그에 새어 들어온 게 2026-09 초에 실제로 지적됐다 — 퇴고 때 「이 문장을 팀원이
  아니라 처음 온 독자가 읽는다」로 한 번 더 본다.
