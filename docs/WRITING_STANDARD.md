...
- **H3**: 제목에 가능하면 실측 수치 1개. **단 H5 를 넘기면서까지 넣지 않는다.**
- **H4 — 한 꼴로 몰리지 않는다.** 2026-08-21 측정: 28편 **전부**가 `A — B` 였다.
  개별 제목이 나빠서가 아니라 **28편이 같은 꼴이라 공식으로 읽힌다.** 사람이 쓴 국내 테크블로그
  표본에서 대시형은 6편 중 1편이었다. **대시형은 전체의 절반을 넘기지 않는다** — 세면 된다.
  - 대안 꼴: 단언형("claude.exe는 리눅스 바이너리였다") · 결과형("1,000건 회귀를 버튼 하나로") ·
    부정형("Figma는 '바뀐 화면 목록'을 주지 않는다") · 질문형("상용 도구로 충분한가")
  - **예외: 시리즈는 병렬 작명이 기능이다.** 「AI ○○ 팀원 만들기」 넷은 같은 꼴이어야 시리즈로 읽힌다.
- **H5 — 길이.** 표본 평균 26자. **35자 이내를 목표**로 한다(2026-08-21 기준 우리 평균 42자 → 32자).
  대시 오른쪽이 왼쪽을 되풀이하면 **오른쪽을 지운다.** 왼쪽만으로 서면 그게 제목이다.
- **H6 — 슬러그는 안 바꾼다.** 제목을 고쳐도 `slug` 은 그대로 둔다. 링크가 죽고 유입이 끊긴다.
